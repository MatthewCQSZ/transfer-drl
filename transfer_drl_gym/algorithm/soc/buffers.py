...
class OptionReplayBufferSamples(NamedTuple):
...
class OptionReplayBuffer(BaseBuffer):
    """
    ...
    def __init__(
        ...
        self.options = np.zeros((self.buffer_size, self.n_envs), dtype=np.int32)
    ...
    def add(
        ...
        option = option.reshape((self.n_envs, 1))
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> OptionReplayBufferSamples:
        ...
        data = (
            ...
            self.options[batch_inds, env_indices],
        )
        tup = (self.to_torch(data[0]), self.to_torch(data[1]), self.to_torch(data[2]), self.to_torch(data[3]), self.to_torch(data[4]), data[5])
        return OptionReplayBufferSamples(*tup)
