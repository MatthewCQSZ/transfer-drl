...
def policy_reuse(old_policy:BasePolicy, env:GymEnv, K, H, phi, mu, gamma, alpha):
    reward = 0
    w = 0
    w_gamma = 1
    done = False
        ...
        for __ in range(H):
            if not done:
                if np.random.rand() < phi:
                    actions = old_policy.predict(obs, deterministic=False)
                else:
                    epsilon = 1 - phi
                    if np.random.rand() < epsilon:
                        actions = new_policy.predict(obs, deterministic=False)
                    else:
