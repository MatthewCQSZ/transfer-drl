...
OffPolicyAlgorithmPolicyReuseSelf = TypeVar("OffPolicyAlgorithmPolicyReuseSelf", bound="OffPolicyAlgorithmPolicyReuse")
...
class OffPolicyAlgorithmPolicyReuse(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.num_old_policy = 0
        self.num_new_policy = 0
        self.num_random = 0
        
        #if old_policy == None:
        #    raise ValueError("No teacher policy selected.")
    ...
    def learn(
        self: OffPolicyAlgorithmPolicyReuseSelf,
        total_timesteps: int,
        callback: MaybeCallback = None,
        log_interval: int = 4,
        ...
        reset_num_timesteps: bool = True,
        progress_bar: bool = False,
    ) -> OffPolicyAlgorithmPolicyReuseSelf:
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            if np.random.rand() < self.reuse_phi:       
                ...
                if np.random.rand() < epsilon:
                    ...
                    self.num_new_policy += 1     
                else:
                    unscaled_action = np.array([np.random.rand(self.env.action_space.shape[0]) for _ in range(n_envs)])
                    self.num_random += 1
                    
                if self.num_timesteps % 5 == 0 and self.verbose != 0:
                    self.logger.record("transfer/num_old_policy", self.num_old_policy)
                    self.logger.record("transfer/num_new_policy", self.num_new_policy)
                    self.logger.record("transfer/num_random", self.num_random)
                    self.logger.record("transfer/reuse_phi", self.reuse_phi)
                    self.logger.record("transfer/epsilon", epsilon)
